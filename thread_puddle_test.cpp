...
{
    println("Checking that pushTask() works for a function with no arguments or return value...");
    {
        bool flag = false;
    ...
    println("Checking that pushTask() works for a function with one argument and no return value...");
    {
        bool flag = false;
    ...
    println("Checking that pushTask() works for a function with two arguments and no return value...");
    {
        bool flag1 = false;
        bool flag2 = false;
...
{
    println("Checking that pushTask() works for a member function with no arguments or return value...");
    {
        flag_class flag;
    ...
    println("Checking that pushTask() works for a member function with one argument and no return value...");
    {
        flag_class flag;
...
{
    println("Checking that pushTask() works within an object for a member function with no arguments or return value...");
    {
        flag_class flag;
        flag.push_test_flag_no_args();
    }
    println("Checking that pushTask() works within an object for a member function with one argument and no return value...");
    {
        flag_class flag;
...
/**
 * @brief Check that pushLoop() works for a specific range of indices split over a specific number of tasks, with no return value.
 *
 ...
 */
template <typename T>
void check_push_loop_no_return(const int64_t random_start, T random_end, const TP::concurrency_t num_tasks)
{
    if (random_start == random_end)
        ++random_end;
    println("Verifying that pushLoop() from ", random_start, " to ", random_end, " with ", num_tasks, num_tasks == 1 ? " task" : " tasks", " modifies all indices...");
...
/**
 * @brief Check that pushLoop() works using several different random values for the range of indices and number of tasks.
 */
void check_parallelize_loop()
{
    ...
    println("Verifying that pushLoop() with identical start and end indices does nothing...");
    bool flag = true;
    const int64_t index = index_dist(mt);
    pool.pushLoop(index, index, [&flag](const int64_t, const int64_t) { flag = false; });
    pool.waitForTasks();
    check(flag);
    println("Trying pushLoop() with start and end indices of different types:");
    ...
    println("Trying the overload for pushLoop() for the case where the first index is equal to 0:");
...
void do_tests()
{
    ...
    print_header("Checking that pushLoop() works:");
...
int main()
{
    println("TP::ThreadPuddle: a fast, lightweight, and easy-to-use C++17 thread pool library");

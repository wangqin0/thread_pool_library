 ...
 *
 * @brief TP::ThreadPool: a fast, lightweight, and easy-to-use C++17 thread pool library. This program tests all aspects of the library, but is not needed in order to use the library.
 */
...
TP::SyncedStream sync_cout(std::cout);
std::ofstream log_file;
TP::SyncedStream sync_file(log_file);
...
{
    dual_print(std::forward<T>(items)..., TP::SyncedStream::endl);
}
...
TP::concurrency_t count_unique_threads()
{
    const TP::concurrency_t num_tasks = pool.getThreadCount() * 2;
    std::vector<std::thread::id> thread_IDs(num_tasks);
    std::unique_lock<std::mutex> total_lock(total_mutex);
    TP::concurrency_t total_count = 0;
    ...
    total_cv.wait(total_lock, [&total_count] { return total_count == pool.getThreadCount(); });
    {
    ...
    return static_cast<TP::concurrency_t>(std::unique(thread_IDs.begin(), thread_IDs.end()) - thread_IDs.begin());
}
...
void check_reset()
{
    ...
    check(std::thread::hardware_concurrency() / 2, pool.getThreadCount());
...
/**
 * @brief Check that pushTask() works.
 */
void check_push_task()
{
    dual_println("Checking that pushTask() works for a function with no arguments or return value...");
    {
        bool flag = false;
    ...
    dual_println("Checking that pushTask() works for a function with one argument and no return value...");
    {
        bool flag = false;
    ...
    dual_println("Checking that pushTask() works for a function with two arguments and no return value...");
    {
        bool flag1 = false;
        bool flag2 = false;
...
public:
    ...
    {
        pool.pushTask(&flag_class::set_flag_no_args, this);
        pool.waitForTasks();
        check(get_flag());
    }
    ...
    {
        pool.pushTask(&flag_class::set_flag_one_arg, this, true);
        pool.waitForTasks();
        check(get_flag());
    }
...
{
    dual_println("Checking that pushTask() works for a member function with no arguments or return value...");
    {
        flag_class flag;
        pool.pushTask(&flag_class::set_flag_no_args, &flag);
        pool.waitForTasks();
        check(flag.get_flag());
    }
    dual_println("Checking that pushTask() works for a member function with one argument and no return value...");
    {
        flag_class flag;
        pool.pushTask(&flag_class::set_flag_one_arg, &flag, true);
        pool.waitForTasks();
        check(flag.get_flag());
    }
...
{
    dual_println("Checking that pushTask() works within an object for a member function with no arguments or return value...");
    {
        flag_class flag;
        flag.push_test_flag_no_args();
    }
    dual_println("Checking that pushTask() works within an object for a member function with one argument and no return value...");
    {
        flag_class flag;
...
/**
 * @brief Check that waitForTasks() works.
 */
void check_wait_for_tasks()
{
    const TP::concurrency_t n = pool.getThreadCount() * 10;
    std::unique_ptr<std::atomic<bool>[]> flags = std::make_unique<std::atomic<bool>[]>(n);
    for (TP::concurrency_t i = 0; i < n; ++i)
        pool.pushTask(
                [&flags, i] {
...
/**
 * @brief Check that pushLoop() or parallelizeLoop() work for a specific range of indices split over a specific number of tasks, with no return value.
 *
 ...
 * @param use_push Whether to check pushLoop() instead of parallelizeLoop().
 */
template <typename T>
void check_parallelize_loop_no_return(const int64_t random_start, T random_end, const TP::concurrency_t num_tasks, const bool use_push = false)
{
    if (random_start == random_end)
        ++random_end;
    dual_println("Verifying that ", use_push ? "pushLoop()" : "parallelizeLoop()", " from ", random_start, " to ", random_end, " with ", num_tasks, num_tasks == 1 ? " task" : " tasks", " modifies all indices...");
    const auto num_indices = static_cast<size_t>(std::abs(random_end - random_start));
    ...
    {
        if (random_start == 0)
            pool.pushLoop(random_end, loop, num_tasks);
        else
            pool.pushLoop(random_start, random_end, loop, num_tasks);
        pool.waitForTasks();
    }
    else
    {
        if (random_start == 0)
            pool.parallelizeLoop(random_end, loop, num_tasks).wait();
        else
            pool.parallelizeLoop(random_start, random_end, loop, num_tasks).wait();
    }
    bool all_flags = true;
...
/**
 * @brief Check that parallelizeLoop() works for a specific range of indices split over a specific number of tasks, with a return value.
 *
 * @param random_start The first index in the loop.
 * @param random_end The last index in the loop plus 1.
 * @param num_tasks The number of tasks.
 */
void check_parallelize_loop_return(const int64_t random_start, int64_t random_end, const TP::concurrency_t num_tasks)
{
    if (random_start == random_end)
        ++random_end;
    dual_println("Verifying that parallelizeLoop() from ", random_start, " to ", random_end, " with ", num_tasks, num_tasks == 1 ? " task" : " tasks", " correctly sums all indices...");
    ...
    const std::vector<int64_t> sums_vector = (random_start == 0) ? pool.parallelizeLoop(random_end, loop, num_tasks).get() : pool.parallelizeLoop(
            random_start, random_end, loop, num_tasks).get();
    int64_t sum = 0;
...
/**
 * @brief Check that pushLoop() and parallelizeLoop() work using several different random values for the range of indices and number of tasks.
 */
void check_parallelize_loop()
{
    std::mt19937_64 mt(rd());
    std::uniform_int_distribution<int64_t> index_dist(-1000000, 1000000);
    std::uniform_int_distribution<TP::concurrency_t> task_dist(1, pool.getThreadCount());
    ...
    dual_println("Verifying that parallelizeLoop() with identical start and end indices does nothing...");
    bool flag = true;
    const int64_t index = index_dist(mt);
    pool.parallelizeLoop(index, index, [&flag](const int64_t, const int64_t) { flag = false; }).wait();
    check(flag);
    dual_println("Trying parallelizeLoop() with start and end indices of different types:");
    const int64_t start = index_dist(mt);
    const auto end = static_cast<uint32_t>(std::abs(index_dist(mt)));
    check_parallelize_loop_no_return(start, end, task_dist(mt));
    dual_println("Trying the overloads for pushLoop() and parallelizeLoop() for the case where the first index is equal to 0:");
...
{
    TP::concurrency_t n = std::min<TP::concurrency_t>(std::thread::hardware_concurrency(), 4);
    ...
    for (TP::concurrency_t i = 0; i < n * 3; ++i)
        pool.pushTask(
                [&release, i] {
                    while (!release[i])
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n * 3 && pool.getTasksRunning() == n && pool.getTasksQueued() == n * 2);
    for (TP::concurrency_t i = 0; i < n; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n * 2 && pool.getTasksRunning() == n && pool.getTasksQueued() == n);
    for (TP::concurrency_t i = n; i < n * 2; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n && pool.getTasksRunning() == n && pool.getTasksQueued() == 0);
    for (TP::concurrency_t i = n * 2; i < n * 3; ++i)
        release[i] = true;
    std::this_thread::sleep_for(sleep_time);
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == 0 && pool.getTasksRunning() == 0 && pool.getTasksQueued() == 0);
...
void check_pausing()
{
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n * 3 && pool.getTasksRunning() == 0 && pool.getTasksQueued() == n * 3);
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n * 2 && pool.getTasksRunning() == n && pool.getTasksQueued() == n);
    dual_println("Pausing pool and using waitForTasks() to wait for the running tasks.");
    pool.pause();
    pool.waitForTasks();
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n && pool.getTasksRunning() == 0 && pool.getTasksQueued() == n);
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    ...
    dual_print("Result: ", pool.getTasksTotal(), " tasks total, ", pool.getTasksRunning(), " tasks running, ",
               pool.getTasksQueued(), " tasks queued ");
    check(pool.getTasksTotal() == n && pool.getTasksRunning() == 0 && pool.getTasksQueued() == n);
    dual_println("Unpausing pool and using waitForTasks() to wait for all tasks.");
    pool.unpause();
    pool.waitForTasks();
...
void check_exceptions()
{
    ...
    dual_println("Checking that exceptions are forwarded correctly by TP::MultiFuture...");
    caught = false;
    TP::MultiFuture<void> my_future2;
    my_future2.pushBack(pool.submit(throws));
    my_future2.pushBack(pool.submit(throws));
    try
    {
        void(my_future2.get());
 ...
 */
void check_vector_of_size(const size_t vector_size, const TP::concurrency_t num_tasks)
{
...
void do_tests()
{
    ...
    print_header("Checking that pushTask() works:");
    check_push_task();
    ...
    print_header("Checking that waitForTasks() works...");
    check_wait_for_tasks();

    print_header("Checking that pushLoop() and parallelizeLoop() work:");
 ...
 */
void print_timing(const TP::concurrency_t num_tasks, const std::pair<double, double>& mean_sd)
{
    if (num_tasks == 0)
 ...
 */
void print_speedup(const std::vector<double>& timings, const TP::concurrency_t try_tasks[])
{
    const auto min_el = std::min_element(std::begin(timings), std::end(timings));
    const double max_speedup = std::round(timings[0] / *min_el * 10) / 10;
    const TP::concurrency_t num_tasks = try_tasks[min_el - std::begin(timings)];
...
void check_performance()
{
    ...
    const TP::concurrency_t thread_count = pool.getThreadCount();
    dual_println("Using ", thread_count, " threads.");

    // Define the number of tasks to try in each run of the test (0 = single-threaded).
    const TP::concurrency_t try_tasks[] = {0, thread_count / 4, thread_count / 2, thread_count, thread_count * 2, thread_count * 4};
    ...
    do
    {
        ...
        tmr.start();
        pool.pushLoop(num_vectors, loop);
        pool.waitForTasks();
        tmr.stop();
    ...
    for (TP::concurrency_t n : try_tasks)
    {
        for (size_t r = 0; r < repeat; ++r)
        {
            ...
            if (n > 1)
            {
                pool.pushLoop(num_vectors, loop, n);
                pool.waitForTasks();
            }
            else
            {
...
int main()
{
    ...
    dual_println("TP::ThreadPool: a fast, lightweight, and easy-to-use C++17 thread pool library");
